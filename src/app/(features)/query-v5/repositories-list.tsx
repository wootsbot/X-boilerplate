...
import type { Repository } from '#/state/queries/github';

export function RepositoriesList({ items }: { items: Repository[] }) {
  return <div className="flex flex-col gap-4">{items?.map((repo) => <Repository key={repo.id} {...repo} />)}</div>;
}
